...
saveas(gcf,'simulated_aCLF_Trajectories.png')

%Plot approach to theta-dependent state
figure;
hold on;
for sim_index = 1:n_sims % Plot sims
    x_target = (-2.5*th0(sim_index) - (13/4));
    x_i = x(sim_index,:);
    norm_i = [];
    for k = 1:length(x_i)
        norm_i(k) = norm(x_i(k)-x_target);
    end
    plot(norm_i);
end
xlabel('Simulation Time (k)')
ylabel('$ \| x_k - x_T(\theta) \|$$','Interpreter','latex')
title('Distance to theta-dependent target over time')
saveas(gcf,'simulated_aCLF_approach_to_sd_goal.png')
