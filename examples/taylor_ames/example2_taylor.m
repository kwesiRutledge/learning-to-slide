...
monom2 = monomials([v1;D1],[0:2]);
[Program1,u1] = sospolyvar(Program1,monom2);

dha_dx(1,1) = diff(ha,v1);
dha_dx(2,1) = diff(ha,D1);

dha_dth(1,1) = diff(ha,f1);
dha_dth(2,1) = diff(ha,f2);
dha_dth(3,1) = diff(ha,f3);

% Create constraints
theta_lb = acc1.theta - dtheta';
theta_ub = acc1.theta + dtheta';

u2 = acc1.m * ( K_D * (D1 - D_star) + K_v * (v1 - v_star) );

tempExpr =  dha_dx'* ( acc1.f([v1;D1]) + acc1.F([v1;D1])* ( theta_lb - Gamma * double(dha_dth) ) + acc1.g([v1;D1])* u_min) - l1 ;
Program1 = sosineq( Program1 , tempExpr ); %F3

tempExpr =  dha_dx'* ( acc1.f([v1;D1]) + acc1.F([v1;D1])* ( theta_ub - Gamma * double(dha_dth) ) + acc1.g([v1;D1])* u_min) - l2 ;
Program1 = sosineq( Program1 , tempExpr ); %F3
...
SOL_ha = sosgetsol(Program1,ha); %Getting solution for V
SOL_dha_dx = sosgetsol(Program1,dha_dx); %Getting solution for dha_dx

figure;
fsurf(SOL_ha); % Plot the lyapunov function
axis([0,alpha,0,alpha,-10,alpha.^2]);
xlabel('v'); ylabel('D'); zlabel('h(v,D)')
view(25,-45)
saveas(gcf,'barrier_image_positive_values.png')

% Save solutions for u1 and u2
SOL_u1 = sosgetsol(Program1,u1);
SOL_u2 = sosgetsol(Program1,u2);

% Plot the lyapunov function over time.
x0s = [ [25;100]];
x_h = []; ha_h = [];
for x0_index = [1:size(x0s,2)]
    x0 = x0s(:,x0_index);
    x = [x0];
    ha = [subs(SOL_ha,struct('v1',x0(1),'D1',x0(2)))];

    x_k = x0;
    ha_k = subs(SOL_ha,struct('v1',x_k(1),'D1',x_k(2)));
    ha_k = double(ha_k);
    dt = 0.1;
    T  = 10;
    for k = [0:dt:T-dt]
        %u_k = subs(SOL_u2,struct('v1',x_k(1),'D1',x_k(2)))
        dha_dx0 = subs(SOL_dha_dx,struct('v1',x_k(1),'D1',x_k(2)))';
        dha_dx = double(dha_dx0);

        [u_k, optim_out] = CLF_control( ...
            x_k , ...
            ha_k , ...
            dha_dx , ...
            acc1 ...
        ...
        x_kp1 = x_k + dt * acc1.dynamics(x_k,u_k);
        x = [x,x_kp1];

        ha_kp1 = subs(SOL_ha,struct('v1',x_kp1(1),'D1',x_kp1(2)));
        ha_kp1 = double(ha_kp1);
...
saveas(gcf,'clf_using_barrier_taylorAmes.png')
